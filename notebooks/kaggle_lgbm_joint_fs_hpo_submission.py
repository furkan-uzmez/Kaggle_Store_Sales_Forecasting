...
# %% [markdown]
# # Store Sales — Joint Feature-Group + Nested HPO (LGBM) → `submission.csv`
#
...
# - SQLite: `optuna_lgbm_joint.db` (resume with `load_if_exists`)
# - **CPU default** (standard LightGBM wheels rarely use T4 GPU)
...
class CFG:
    ...
    study_name: str = "lgbm_joint_fs_hpo"
    optuna_db_name: str = "optuna_lgbm_joint.db"
    optuna_load_if_exists: bool = True

    target_transform: str = "log1p"
    clip_negative: bool = True
    use_gpu: bool = False  # CPU default
    n_jobs: int = 4
...
CFG = CFG()
CFG.smoke = os.environ.get("JOINT_NOTEBOOK_SMOKE", "0") == "1"
if CFG.smoke:
    CFG.n_trials = 4
...
if _kaggle_in is not None:
    ...
    WORK_DIR = _kaggle_work if _kaggle_work.exists() else Path("outputs/kaggle_lgbm_joint")
    ON_KAGGLE = True
else:
    INPUT_DIR = Path("data/raw")
    WORK_DIR = Path("outputs/kaggle_lgbm_joint")
    ON_KAGGLE = False
...
def fit_lgbm(
    ...
    if (
        ...
        fit_kw["callbacks"] = [
            lgb.early_stopping(int(early_stopping), verbose=False),
...
def predict_lgbm(model: lgb.LGBMRegressor, X: pd.DataFrame) -> np.ndarray:
    return inverse_y(model.predict(X))
...
#
# Trial θ = (use_oil, use_holiday, use_store_meta, use_transactions, lgbm_params).  
...
BEST_PARAMS = bp  # LGBM hparams only
...
def outer_cv_score(seed: int) -> dict[str, Any]:
    ...
    for fold in OUTER_FOLDS:
        ...
        pred = predict_lgbm(model, X_va.loc[ok_v])
...
# %%
def recursive_predict_test(model: lgb.LGBMRegressor) -> pd.DataFrame:
    ...
    for day in test_dates:
        ...
        pred = predict_lgbm(model, day_rows[BEST_FEAT_COLS])
...
for s in CFG.seeds:
    ...
    model.booster_.save_model(str(WORK_DIR / f"lgbm_joint_seed{s}.txt"))
    del model
    gc.collect()
...
# | Step | Artifact |
# | --- | --- |
# | Joint FS+HPO | `optuna_lgbm_joint.db`, `trials.csv`, `joint_hpo_summary.json` |
