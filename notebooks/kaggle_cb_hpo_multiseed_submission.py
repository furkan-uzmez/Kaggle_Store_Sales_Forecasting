...
# %% [markdown]
# # Store Sales — CatBoost Nested HPO → Multi-Seed → `submission.csv` (Kaggle-ready)
#
...
# **Local reference:** CB ladder seed-42 public LB ≈ **0.46567** (no HPO). This notebook adds nested HPO.
#
# **Runtime notes:**
# - Default `CFG.n_trials = 25` (CatBoost is slower than LGBM/XGB per trial). Raise to 40 if time allows.
# - Optuna trials persist to **`optuna_cb_store_sales.db`** under working dir (resume if file kept).
# - Trial iterations capped; full retrain uses more iterations + early stopping.
# - GPU: optional (`CFG.use_gpu=True` probes CatBoost GPU, falls back to CPU). T4 may help.
# - Internet not required if Optuna/CatBoost are preinstalled on the image.
...
class CFG:
    ...
    # Nested HPO
    n_trials: int = 25  # CatBoost costlier per trial; raise to 40 if time allows
    ...
    study_name: str = "cb_store_sales_nested"
    optuna_db_name: str = "optuna_cb_store_sales.db"
    ...
    # Device — CatBoost may use GPU; probe + fallback to CPU
    use_gpu: bool = True
    n_jobs: int = 4
    thread_count: int = 4

    # Smoke mode (tiny subsample — off by default; set env CB_NOTEBOOK_SMOKE=1)
    smoke: bool = False
    smoke_max_entities: int = 40


CFG = CFG()
CFG.smoke = os.environ.get("CB_NOTEBOOK_SMOKE", "0") == "1"
if CFG.smoke:
    CFG.n_trials = 2
...
if _kaggle_in is not None:
    ...
    WORK_DIR = _kaggle_work if _kaggle_work.exists() else Path("outputs/kaggle_cb_hpo")
    WORK_DIR.mkdir(parents=True, exist_ok=True)
    ON_KAGGLE = True
else:
    # Local repo: data/raw
    INPUT_DIR = Path("data/raw")
    WORK_DIR = Path("outputs/kaggle_cb_hpo")
    WORK_DIR.mkdir(parents=True, exist_ok=True)
    ON_KAGGLE = False
...
def cb_task_type() -> str:
    """Probe CatBoost GPU; fall back to CPU."""
    if not CFG.use_gpu:
        return "CPU"
    try:
        ...
        print("CatBoost GPU unavailable, using CPU:", exc)
        return "CPU"


TASK_TYPE = cb_task_type()
print("CatBoost task_type:", TASK_TYPE, "(use_gpu=%s)" % CFG.use_gpu)
...
# %% [markdown]
# ## 6. CatBoost fit / predict helpers
#
# Nested temporal HPO search space: depth, learning_rate, l2_leaf_reg, random_strength, subsample.

# %%
# Columns treated as categorical for CatBoost (string cast at fit/predict).
CB_CAT_COLS = ["family_code"]  # family encoded as code; also pass store_nbr as numeric


def make_catboost(params: dict[str, Any], seed: int, n_estimators: int) -> CatBoostRegressor:
    p = dict(params)
    p.setdefault("learning_rate", 0.05)
    p.setdefault("depth", 8)
    p.setdefault("l2_leaf_reg", 3.0)
    p.setdefault("random_strength", 1.0)
    p.setdefault("subsample", 0.8)
    # Drop keys CatBoost does not accept if leaked from other models
    ...
    if TASK_TYPE == "GPU":
        kwargs["devices"] = "0"
        # subsample on GPU can be restricted; keep if provided
    return CatBoostRegressor(**kwargs)


def _prepare_cb_frame(X: pd.DataFrame) -> pd.DataFrame:
    """Cast cat-like columns to str for CatBoost."""
    out = X.copy()
...
def fit_cb_model(
    ...
    global TASK_TYPE
    model = make_catboost(params, seed=seed, n_estimators=n_estimators)
    y_tr_t = transform_y(y_tr)
    X_tr_p = _prepare_cb_frame(X_tr)
    cat_features = [c for c in CB_CAT_COLS if c in X_tr_p.columns]
    fit_kw: dict[str, Any] = {}
    if cat_features:
        fit_kw["cat_features"] = cat_features
    if (
        X_va is not None
        ...
        X_va_p = _prepare_cb_frame(X_va)
        fit_kw["eval_set"] = (X_va_p, transform_y(y_va))
    ...
    try:
        ...
        if TASK_TYPE != "GPU":
            raise
        print("CatBoost GPU fit failed, retry CPU:", exc)
        # Rebuild on CPU (set_params alone is unreliable across CB versions)
        TASK_TYPE = "CPU"
        model = make_catboost(params, seed=seed, n_estimators=n_estimators)
        model.fit(X_tr_p, y_tr_t, **fit_kw)
    return model
...
def predict_cb(model: CatBoostRegressor, X: pd.DataFrame) -> np.ndarray:
    pred_t = model.predict(_prepare_cb_frame(X))
...
# - Path: `WORK_DIR / optuna_cb_store_sales.db` (e.g. `/kaggle/working/optuna_cb_store_sales.db`)
# - `load_if_exists=True` → re-running HPO **resumes** and only schedules remaining trials up to `n_trials`
# - Also exports `trials.csv` + `cb_hpo_summary.json` for easy inspection
#
...
def run_hpo(n_trials: int) -> optuna.Study:
    def objective(trial: optuna.Trial) -> float:
        # CatBoost-oriented nested temporal search space
        params = {
...
hpo_path = WORK_DIR / "cb_hpo_summary.json"
hpo_path.write_text(
    json.dumps(
        {
...
def outer_cv_score(params: dict[str, Any], seed: int) -> dict[str, Any]:
    ...
    for fold in OUTER_FOLDS:
        ...
        pred = predict_cb(model, X_va.loc[va_ok])
...
multi_df.to_csv(WORK_DIR / "cb_multiseed_outer.csv", index=False)
...
for s in CFG.seeds:
    ...
    if best_it is None and hasattr(model, "get_best_iteration"):
        try:
    ...
    # Save model
    model_path = WORK_DIR / f"cb_seed{s}.cbm"
...
card = {
    ...
    "n_trials_complete": _n_complete_trials(study),
    "n_trials_all_states": len(study.trials),
    "n_trials_target": 25 if not CFG.smoke else 2,
    "optuna_db": str(OPTUNA_DB_PATH),
    "optuna_storage": OPTUNA_STORAGE,
    "study_name": CFG.study_name,
    "task_type": TASK_TYPE,
    ...
    "n_rows": int(len(sub)),
    "search_space": "learning_rate, depth, l2_leaf_reg, random_strength, subsample",
}
...
# | Step | Result |
# | --- | --- |
# | Nested HPO (25 trials default) | SQLite `optuna_cb_store_sales.db` + `trials.csv` + `cb_hpo_summary.json` |
# | Multi-seed outer | `cb_multiseed_outer.csv` — seeds 42/43/44 |
# | Submission | Seed-mean recursive preds → **`submission.csv`** |
# | Compare | Public LB vs prior CB **0.46567**, XGB **0.44139**, LGBM **0.47064** |
#
# **Observation:** Nested HPO never peeks outer val or public LB; trials are DB-backed.  
# **Interpretation:** HPO may close the gap vs prior non-tuned CB submission.  
# **Action:** Keep/download the `.db` to resume; update `docs/kaggle_public_scores.md` after submit.
